...
void	draw_ray_wall(t_ray *ray, t_game *game)
{
	...
	printf("height : %d\n", draw_end - draw_start);
	printf("camera dirx : %f, diry : %f, posx : %f, posy : %f, planx : %f, plany : %f\n", game->camera.dir_x, game->camera.dir_y, game->camera.pos_x, game->camera.pos_y, game->camera.plan_x, game->camera.plan_y);
	printf("player : %d\n", game->map.content[(int)floor(game->camera.pos_y)][(int)floor(game->camera.pos_x)]);
	printf("hit %d\n", ray->hit);
	if (ray->hit)
	{
		i = draw_start;
		while (i < draw_end)
		{
			mlx_set_image_pixel(game->screen.mlx, game->screen.img, ray->pos_x, i, color);
			i++;
		}
...
void	ray_move(t_ray *ray, t_game *game)
{
	...
	while (!ray->hit)
	{
		...
		if (ray->map_x < 0 || game->map.width < ray->map_x || ray->map_y < 0 || game->map.height < ray->map_y)
		{
			if (ray->side_dist_x < ray->side_dist_y)
				ray->map_x -= ray->step_x;
			else
